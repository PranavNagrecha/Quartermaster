...
- `config: RouterConfig` — `ranker`, `synonyms`, `stopwords`, `nameWeight`, `k1`, `b`, `expansionWeight`, `hintBoost`, `marginThreshold`, `minTopScore`.
...
Same query + same corpus ⇒ same candidate order every time. Tie-breaks use
`localeCompare` on tool name (ascending). Synonym expansion order follows object
key iteration order in the supplied `synonyms` map.
